...
[VolumeComponentMenu("Post-processing Custom/Posterize")]

// If the related Scriptable Renderer Feature doesn't exist, display a warning about adding it to the renderer.
[VolumeRequiresRendererFeatures(typeof(PosterizeRendererFeature))]
...
public sealed class PosterizeVolumeComponent : VolumeComponent, IPostProcessComponent
{
    // Set the name of the volume component in the list in the Volume Profile.
    public PosterizeVolumeComponent()
    {
    ...
    [Tooltip("Intensity of posterize")]
    public FloatParameter level = new FloatParameter(0f);
